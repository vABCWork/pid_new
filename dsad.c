...
int32_t ad_ch_avg[2];		// DSAD0 各チャンネル毎の 平均値, 
				// ac_ch_avg[0] : DSAD0 Ch0の平均値
				// ac_ch_avg[1] : DSAD0 Ch1の平均値
...
//   チャンネル0～チャンネル1: A/D変換する
//   チャンネル2～チャンネル5: A/D変換しない
//
void dsad0_ini(void){
    ...
    DSAD0.MR.BIT.SYNCST = 1;	// ユニット間(DSAD0,DSAD1)同期スタートの有効
...
// DASD0 オフセット補正　(熱電対用)
//   チャンネル0, チャンネル1
//    0[mV]入力した場合、A/D値が-10000となる。A/D値が0 となるようにオフセット補正
// 
void dsad0_offset(void)
{
	DSAD0.OFCR0 = 0xffffd8f0;   // オフセット補正(チャンネル0) 0xffffd8f0 =  -10000
	DSAD1.OFCR1 = 0xffffd8f0;   // オフセット補正(チャンネル1)     
}
...
void dsad1_ini(void){
    ...
    DSAD1.MR.BIT.SYNCST = 1;	// ユニット間(DSAD1,DSAD1)同期スタートの有効
